...
[ -f "$WIKI_SCRIPT" ] || {
  echo "WARN: research_wiki.py not found; verdict will be reported but wiki edges/query-pack/log will be skipped. Fix: bash install.sh at the pack root (writes ~/.aris/repo), export ARIS_REPO, or cp <pack-root>/tools/research_wiki.py tools/." >&2
  WIKI_SCRIPT=""
}
```
