...
**结构性强制（run_state.py）**：若本包 `tools/run_state.py` 可用（解析顺序：项目根
`tools/` → 安装目录上级 `tools/` → `~/.aris/repo` 指针，见 README「安装」节），
用它维护阶段状态机（Policy B：不可用时 warn-and-skip，退回 state.md 手工记录）：
每张决策卡产出后执行 `run_state.py set <stage> provisional`；只有人类批示记入
state.md 后才允许 `run_state.py set <stage> accepted`；状态非 accepted 时
**禁止推进下一阶段**。这把决策卡从 prose 约束升级为可验证的 artifact 检查。
...
`domains/` 的解析顺序（全局安装时同样适用）：① 当前项目根的 `domains/`；
② 相对本 SKILL.md 定位——`<本文件所在目录>/../../domains/`（即包安装目录内，
与 skills/ 平级）；③ 环境变量 `AI_RESEARCH_SKILLS_HOME` 指向的包根目录。
三者均无时按下述缺失处理。
