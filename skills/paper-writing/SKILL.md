...
- CCF-A story-first structure & claim-evidence map (from this pack's pipeline): `references/ccfa-structure.md` — read FIRST when drafting a new paper from pipeline artifacts (evidence_summary.md / hypothesis_tree.md); it defines the story-freeze step and the claim-evidence hard constraint used by downstream reviewer-simulation.
